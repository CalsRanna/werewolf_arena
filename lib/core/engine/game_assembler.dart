...
class GameAssembler {
  ...
  /// 3. 默认使用9人局
  static Future<GameScenario> _selectScenario(String? scenarioId, int? playerCount) async {
    // 根据scenarioId选择
    if (scenarioId != null) {
  ...
  static PlayerIntelligence _getPlayerIntelligence(GameConfig config, int playerIndex) {
    // 尝试获取指定玩家的智能配置
  ...
  static bool validateScenario(GameScenario scenario) {
    ...
    final hasWerewolf = roles.any((role) => 
      role == RoleType.werewolf);
    final hasGoodGuy = roles.any((role) => 
      role != RoleType.werewolf);
