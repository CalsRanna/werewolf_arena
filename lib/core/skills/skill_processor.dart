...
class SkillProcessor {
  ...
  Future<void> _resolveProtectionConflicts(
    ...
    for (final killResult in killResults) {
      ...
      // 检查是否有保护效果
      final isProtected = protectResults.any((protectResult) => 
        protectResult.target == target
      );
  ...
  Future<void> _resolveHealingConflicts(
    ...
    for (final killResult in killResults) {
      ...
      // 检查是否有治疗效果
      final isHealed = healResults.any((healResult) => 
        healResult.target == target
      );
  ...
  /// 撤销击杀效果
  Future<void> _cancelKillEffect(SkillResult killResult, GameState state) async {
    final target = killResult.target;
  ...
  bool _isHealSkill(SkillResult result) {
    final skillId = result.metadata['skillId'] as String?;
    return skillId != null && (
      skillId.contains('heal') || 
      skillId.contains('antidote')
    );
  }
}