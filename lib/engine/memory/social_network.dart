...
import 'package:werewolf_arena/engine/memory/relationship.dart';
...
class SocialNetwork {
  ...
  /// 更新对某个玩家的关系
  SocialNetwork updateRelationship(String playerId, Relationship newRelationship) {
    return SocialNetwork(
      ownerId: ownerId,
  ...
  List<String> getMostSuspicious({int limit = 3}) {
    final sorted = relationships.entries.toList()
      ..sort((a, b) => b.value.suspicionLevel.compareTo(a.value.suspicionLevel));
  ...
  String toPrompt(GameState state) {
    ...
    if (enemies.isNotEmpty) {
      final enemyNames = enemies.map((id) {
        final player = state.players.firstWhere(
          (p) => p.id == id,
          orElse: () => state.players.first,
        );
  ...
  String toCompactPrompt(GameState state) {
    ...
    if (topSuspicious.isNotEmpty) {
      ...
      final names = topSuspicious.map((id) {
        final player = state.players.firstWhere(
          (p) => p.id == id,
          orElse: () => state.players.first,
        );
