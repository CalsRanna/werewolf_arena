...
class IdentityInferenceStep extends ReasoningStep {
  ...
  Future<ReasoningContext> execute(
    ...
    try {
      ...
      final identityEstimates = _parseResponse(response);
...
''';
  ...
  /// 解析LLM响应
  Map<String, IdentityEstimate> _parseResponse(String response) {
    try {
      ...
      final estimates = <String, IdentityEstimate>{};
      json.forEach((playerName, data) {
        if (data is Map<String, dynamic>) {
          estimates[playerName] = IdentityEstimate(
      ...
      GameEngineLogger.instance.e('[身份推理] JSON解析失败: $e\nResponse: $response');
      return {};
    }
  }
}
