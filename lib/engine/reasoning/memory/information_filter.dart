...
class InformationFilter {
  ...
  String buildFilteredContext(
    ...
    buffer.writeln('- 出局玩家：${state.deadPlayers.map((p) => p.name).join(", ")}');
  ...
  ///
  /// 未来优化：分析发言和投票记录
  bool _hasAttackedMe(GamePlayer player, GamePlayer me, GameState state) {
    // 当前返回false，依赖LLM从发言中推理
    return false;
  }
  ...
  KeyFact? _eventToKeyFact(GameEvent event, GameState state) {
    ...
    if (narrative.contains('死亡') || narrative.contains('出局')) {
      return KeyFact(description: narrative, importance: 90, day: event.day);
    }
