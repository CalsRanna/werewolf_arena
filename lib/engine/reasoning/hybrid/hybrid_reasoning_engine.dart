...
import 'package:werewolf_arena/engine/reasoning/hybrid/postprocessing_stage.dart';
import 'package:werewolf_arena/engine/reasoning/hybrid/preprocessing_stage.dart';
...
class HybridReasoningEngine {
  ...
  Future<ReasoningResult> execute({
    ...
    _log('=' * 60);
    _log('开始混合推理: ${player.name} (${player.role.name}) - ${skill.name}');
    _log('=' * 60);
    ...
    try {
      ...
      final worldState = await _preprocessingStage.execute(
        player: player,
        state: state,
        skill: skill,
      );
