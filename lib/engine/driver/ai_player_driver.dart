...
class AIPlayerDriver implements PlayerDriver {
  ...
  String _getMemoryPrompt(GamePlayer player) {
    if (player is! AIPlayer) return '';
    return '''
${player.persona.memoryDirective}
...
3.  **行为逻辑**：${player.persona.behavioralLogic}

4.  **人性化表达**：${player.persona.languageStyle}

5.  **游戏术语**：${player.persona.gameSlang}

6.  **情境反应**：${player.persona.situationalReactions}

7.  **构建叙事，而非罗列信息**：不要仅仅陈述"A做了B，所以我怀疑A"。要将零散的公开信息串联成一个有说服力的故事，引导其他玩家的思维。例如："3号玩家昨天说要保4号，今天却第一个投票给4号，他的行为逻辑已经完全崩塌了，在我这里他就是一张必出的狼人牌。" 你的秘密信息是你的剧本大纲，但你台上的表演，必须让观众觉得合情合理。

8.  **善用高级战术**：逻辑分析只是基础。心理博弈、话术陷阱、建立同盟、制造对立、悍跳、倒钩、冲锋... 这些都是你信手拈来的武器。所有高级战术的本质，都是在**隐藏关键信息**和**释放虚假信息**之间找到平衡。根据你的角色和场上局势，灵活运用。

9.  **拥有记忆和立场**：你必须记住之前的回合发生了什么，谁说了什么，谁投了谁。你的每一次发言和决策都必须基于这些记忆，并服务于你当前阵营的立场。保持你人设和逻辑的一致性，除非你在进行战术伪装。
...
''';
    ...
    final memoryPrompt = _getMemoryPrompt(player);

    try {
      final updatedMemory = await _request(prompt, systemPrompt: memoryPrompt);
...
- **关键事件回顾**:
  ...
  Future<String> _request(String prompt, {required String systemPrompt}) async {
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final response = await _call(prompt, systemPrompt: systemPrompt);
