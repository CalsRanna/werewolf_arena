...
void main() {
  group('集成测试 - 完整游戏流程', () {
    test('游戏初始化流程 - GameAssembler创建完整游戏', () async {
      ...
      expect(
        gameEngine.status,
        equals(GameEngineStatus.waiting),
        reason: '引擎初始状态应该为waiting',
      );
    ...
    test('阶段转换逻辑 - Night到Day切换', () async {
      ...
      expect(
        gameEngine.status,
        equals(GameEngineStatus.playing),
        reason: '游戏应该开始运行',
      );
      expect(
        gameEngine.currentState!.currentPhase,
        equals(GamePhase.night),
        reason: '应该从夜晚阶段开始',
      );
      ...
      if (hasNextStep) {
        ...
        // 验证事件历史
        final phaseEvents = gameEngine.currentState!.eventHistory
            .where((e) => e.type.name == 'phaseChange')
            .toList();
    ...
    test('不同场景配置 - 9人局vs12人局', () async {
      ...
      expect(
        game12Engine.scenario.playerCount,
        equals(12),
        reason: '场景玩家数应该为12',
      );
      ...
      expect(
        roles12.length,
        greaterThan(roles9.length),
        reason: '12人局角色应该比9人局多',
      );
    });
    ...
    test('GameEngine核心功能 - 完整的游戏周期', () async {
      ...
      while (await gameEngine.executeGameStep() && stepCount < maxSteps) {
        ...
        expect(
          gameEngine.currentState!.currentPhase,
          isIn([GamePhase.night, GamePhase.day]),
        ...
        expect(
          gameEngine.currentState!.eventHistory.isNotEmpty,
          true,
          reason: '应该有事件历史',
        );
      ...
      expect(stepCount, lessThanOrEqualTo(maxSteps), reason: '测试步骤不应超过最大限制');
    });
    ...
    test('技能系统集成 - 夜晚技能执行', () async {
      ...
      if (gameEngine.currentState!.currentPhase == GamePhase.night) {
        ...
        expect(
          gameEngine.currentState!.eventHistory.length,
          greaterThanOrEqualTo(initialEventCount),
          reason: '夜晚阶段应该保持或增加事件',
        );

        // 验证技能效果
        final skillEffects = gameEngine.currentState!.skillEffects;
        expect(skillEffects, isA<Map<String, dynamic>>(), reason: '应该有技能效果存储');
      }
    });
  ...
  group('集成测试 - 错误处理和边界情况', () {
    ...
    test('玩家数量不匹配处理', () async {
      ...
      expect(
        () async {
          await GameAssembler.assembleGame(scenarioId: 'invalid_scenario');
        },
