  ...
  group('Scenario12Players Tests', () {
    ...
    test('扩展角色列表测试', () {
      ...
      scenario.roleDistribution.forEach((role, expectedCount) {
        expect(counts[role], equals(expectedCount), 
  ...
  group('GameScenario 通用接口测试', () {
    test('9人局和12人局都应该实现完整的GameScenario接口', () {
      ...
      for (final scenario in scenarios) {
        ...
        final total = scenario.roleDistribution.values.fold(0, (sum, count) => sum + count);
        expect(total, equals(scenario.playerCount));
        
        // 扩展角色列表长度正确
        expect(scenario.getExpandedGameRoles().length, equals(scenario.playerCount));
