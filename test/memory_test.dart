...
import 'package:werewolf_arena/engine/events/last_words_event.dart';
...
void main() {
  group('内存使用测试', () {
    test('游戏对象内存占用分析', () async {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        if ((i + 1) % 5 == 0) {
          ...
          print(
            '- 事件历史长度: ${gameEngine.currentState?.eventHistory.length ?? 0}',
          );
          print(
            '- 技能效果数: ${gameEngine.currentState?.skillEffects.length ?? 0}',
          );
        }
      }
      ...
      expect(
        games.every((g) => g.players.isNotEmpty),
        isTrue,
        reason: '所有游戏都应该有玩家',
      );
    });
    ...
    test('事件历史内存增长测试', () async {
          ...
          case 2:
            gameState.handleEvent(
              VoteEvent(
                voter: player,
                candidate: gameState
                    .alivePlayers[(i + 1) % gameState.alivePlayers.length],
      ...
      print(
        '平均添加速度: ${(eventCount / stopwatch.elapsedMilliseconds * 1000).toStringAsFixed(2)} events/sec',
      );

      // 验证事件添加的一致性
      expect(
        finalEventCount,
        greaterThan(initialEventCount),
        reason: '应该成功添加大量事件',
      );
      expect(
    ...
    test('事件查询性能在大数据集下的表现', () async {
      ...
      for (int i = 0; i < 100; i++) {
        final player =
            gameState.alivePlayers[i % gameState.alivePlayers.length];
        final speechEvents = gameState.getEventsByType(
          player,
      ...
      expect(typeQueryTime / 100, lessThan(10000), reason: '类型筛选查询时间应该小于10ms');
    });
    ...
    test('技能效果存储内存优化测试', () async {
      ...
      // 验证清理效果
      expect(gameState.skillEffects.length, equals(0), reason: '技能效果应该被完全清理');
    });
    ...
    test('并发游戏实例内存隔离测试', () async {
        ...
        final otherGameEffects = gameState.skillEffects.keys
            .where(
              (key) =>
                  ...
                  !key.startsWith('game_${gameIndex}_'),
            )
            .toList();
        expect(
          otherGameEffects.length,
          equals(0),
      ...
      for (int gameIndex = 5; gameIndex < 10; gameIndex++) {
        ...
        final remainingEffects = gameState.skillEffects.keys
            .where((key) => key.startsWith('game_${gameIndex}_'))
            .toList();
        expect(remainingEffects.length, equals(100), reason: '剩余游戏的数据应该保持完整');
      }
