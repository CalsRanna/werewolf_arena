...
void main() {
  group('GamePlayer基础功能测试', () {
    ...
    test('知识管理测试', () {
      // 添加和获取知识
      aiPlayer.addKnowledge('seer_result', {'player': 'Player2', 'result': 'Good'});
      expect(aiPlayer.hasKnowledge('seer_result'), isTrue);
      
      final seerResult = aiPlayer.getKnowledge<Map<String, dynamic>>('seer_result');
    ...
    test('死亡处理测试', () {
      ...
      expect(aiPlayer.isAlive, isFalse);
      expect(aiPlayer.getPrivateData('death_cause'), equals(DeathCause.werewolfKill));
    });
  ...
  group('Driver集成测试', () {
    test('AIPlayer应该有AIPlayerDriver', () {
      ...
      expect(aiPlayer.driver.runtimeType.toString(), contains('AIPlayerDriver'));
    });
    ...
    test('HumanPlayer应该有HumanPlayerDriver', () {
      ...
      expect(humanPlayer.driver.runtimeType.toString(), contains('HumanPlayerDriver'));
    });
  });
}
...
class MockSkill extends GameSkill {
  ...
  Future<SkillResult> cast(dynamic player, GameState state) async {
    return castResult ?? SkillResult.success(
      caster: player,
      target: player,
      metadata: {'mock': true},
    );
