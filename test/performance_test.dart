...
void main() {
  group('性能基准测试', () {
    ...
    test('游戏引擎初始化性能 - 9人局', () async {
      ...
      expect(avgTime / 1000, lessThan(50), 
        reason: '游戏初始化平均时间应该小于50ms');
    });
    ...
    test('游戏引擎初始化性能 - 12人局', () async {
      ...
      expect(avgTime / 1000, lessThan(100), 
        reason: '12人局游戏初始化平均时间应该小于100ms');
    });
    ...
    test('技能系统执行性能', () async {
      ...
      expect(avgTime / 1000, lessThan(10), 
        reason: '技能执行平均时间应该小于10ms');
    });
    ...
    test('事件系统处理性能', () async {
      ...
      // 性能断言
      expect(avgAddTime, lessThan(100), 
        reason: '事件添加平均时间应该小于100μs');
      expect(avgQueryTime, lessThan(1000), 
        reason: '事件查询平均时间应该小于1ms');
    });
    ...
    test('大规模游戏循环性能', () async {
      ...
      for (int gameIndex = 0; gameIndex < gameCount; gameIndex++) {
        ...
        print('游戏 ${gameIndex + 1} 完成，执行了 $stepCount 步，用时 ${gameStopwatch.elapsedMilliseconds}ms');
      }
      ...
      print('游戏吞吐量: ${(gameCount * 1000 / totalTime).toStringAsFixed(2)} games/sec');
      
      // 性能断言：限定步骤的游戏应该在合理时间内完成
      expect(avgGameTime, lessThan(5000), 
        reason: '限定步骤的游戏平均时间应该小于5秒');
    });
    ...
    test('内存使用基准测试', () async {
      ...
      // 基本的内存使用检查
      expect(memoryIncrease ~/ 1024 ~/ 1024, lessThan(500), 
